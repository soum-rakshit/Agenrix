...
import { Upload, Download, X, Loader2, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function BulkImportModal({ isOpen, onClose, mode, onSuccess }) {
  ...
  const [results, setResults] = useState(null); // { success: number, failed: number, total: number }
  ...
  const downloadTemplate = () => {
    const headers = [
      'agent_id', 'agent_name', 'agent_source', 'agent_description', 'owner', 
      'authorized_by', 'subscription_plan', 'contributors', 
      'access_tools', 'access_files', 'access_data_nodes', 'access_apis', 'access_servers'
    ].join(',');
    
    const sampleRow = [
      'AGT-CSV-01', 'Bulk Agent', 'Internal', 'Imported via CSV', 'Alpha Team', 
      'Jane Doe', 'Pro', 'alice|bob', 
      'nmap|wireshark', '/etc/passwd', 'DB-Main', 'AuthService', 'srv-01|srv-02'
    ].join(',');
  ...
  const constructPayload = (row) => {
    ...
    // Clean up undefined/null fields if it's an update so we don't overwrite with blanks
    if (mode === 'update') {
  ...
  const handleImport = async () => {
      ...
      const promises = chunk.map(async (row) => {
        ...
        try {
          ...
          if (mode === 'add') {
            ...
            await api.patch(`/update_agent/${row.agent_id}`, payload);
          }
    ...
    setIsUploading(false);
    setResults({ success: successCount, failed: failCount, total: parsedData.length });

    if (failCount === 0) {
      toast.success(`Successfully ${mode === 'add' ? 'imported' : 'updated'} ${successCount} agents!`);
      if (onSuccess) onSuccess();
    } else {
      toast.error(`${successCount} succeeded, ${failCount} failed. Check console for details.`);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-card w-full max-w-4xl rounded-lg shadow-xl border border-border flex flex-col max-h-[90vh]">
        ...
        <div className="flex justify-between items-center p-6 border-b border-border">
          <div>
            ...
            <p className="text-sm text-foreground/60 mt-1">
              Upload a CSV file to process multiple agents at once. Arrays should be pipe-separated (|).
            </p>
          </div>
          <button onClick={handleClose} disabled={isUploading} className="text-foreground/50 hover:text-foreground transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 flex-1 overflow-y-auto">
          ...
          {/* Results Summary */}
          {results && (
            <div className={`p-4 rounded-md mb-6 flex items-start gap-3 ${results.failed > 0 ? 'bg-red-500/10 border border-red-500/20' : 'bg-green-500/10 border border-green-500/20'}`}>
              {results.failed > 0 ? <AlertCircle className="w-5 h-5 text-red-500 mt-0.5" /> : <CheckCircle className="w-5 h-5 text-green-500 mt-0.5" />}
              <div>
                <h4 className={`font-medium ${results.failed > 0 ? 'text-red-500' : 'text-green-500'}`}>
                  Import Completed
                </h4>
                <p className="text-sm text-foreground/80 mt-1">
                  Successfully {mode === 'add' ? 'added' : 'updated'} {results.success} agents. {results.failed > 0 && `${results.failed} failed.`}
                </p>
              </div>
            </div>
          )}
          ...
          {isUploading && (
            <div className="mb-6">
              <div className="flex justify-between text-xs font-medium text-foreground/70 mb-2">
                ...
                <span>{progress} / {parsedData.length}</span>
              </div>
              <div className="w-full bg-foreground/10 rounded-full h-2">
                <div 
                  className="bg-primary h-2 rounded-full transition-all duration-300" 
                  style={{ width: `${(progress / parsedData.length) * 100}%` }}
                ></div>
              </div>
            </div>
          )}
          ...
          {parsedData.length > 0 && !results && (
            <div className="border border-border rounded-md overflow-hidden">
              <div className="bg-foreground/5 px-4 py-2 border-b border-border">
                <h3 className="text-sm font-medium text-foreground">Preview ({parsedData.length} rows)</h3>
              </div>
                  ...
                  <tbody className="divide-y divide-border">
                    {parsedData.slice(0, 10).map((row, i) => (
                      <tr key={i} className="hover:bg-foreground/5 transition-colors">
                        ...
                        <td className="px-4 py-2 truncate max-w-[150px]">{row.agent_name || '-'}</td>
                        <td className="px-4 py-2">{row.owner || '-'}</td>
                        <td className="px-4 py-2">{row.agent_source || '-'}</td>
                        <td className="px-4 py-2 truncate max-w-[150px]">{row.access_tools || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="p-6 border-t border-border flex justify-end gap-3 bg-foreground/5 rounded-b-lg">
          ...
          <button 
            ...
            disabled={isUploading || parsedData.length === 0 || results !== null}
