...
const ALL_EDITABLE_FIELDS = [...EDITABLE_FIELDS.core, ...EDITABLE_FIELDS.access];
...
export default function UpdateAgent() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (searchId.trim()) queryParams.append('agent_id', searchId.trim());
      if (searchName.trim()) queryParams.append('agent_name', searchName.trim());
      ...
      if (response.data && response.data.length > 0) {
        ...
        if (response.data.length > 1) {
          toast.success(`Found ${response.data.length} matching agents. Loaded the first one.`);
        } else {
  ...
  const onSubmit = async (data) => {
    try {
      // Build update payload based on visible fields
      const payload = {};
      ...
      EDITABLE_FIELDS.access.forEach(field => {
        if (visibleFields[field.id]) {
          access_rights[field.id] = data.access_rights?.[field.id] || [];
  ...
  const allFieldsSelected = ALL_EDITABLE_FIELDS.every(field => visibleFields[field.id]);
  ...
  const toggleGroupFields = (groupKey) => {
    const isAllSelected = EDITABLE_FIELDS[groupKey].every(field => visibleFields[field.id]);
  ...
  return (
    <div className="max-w-3xl mx-auto animate-in fade-in duration-500 pb-12">
      <div className="mb-8 flex justify-between items-start">
        <div>
          <h2 className="text-3xl font-bold text-foreground">Update Agent</h2>
          <p className="text-foreground/60 mt-1">Search for an agent by ID or Name to update its configuration and access rights.</p>
        </div>
        <button 
          type="button"
      ...
      <div className="bg-card border border-border p-6 rounded-lg shadow-sm mb-8">
        <form onSubmit={handleSearch} className="flex gap-4 items-end">
          <div className="flex-1">
            <label className="block text-sm font-medium text-foreground mb-1">Search Agent ID</label>
            <input
              type="text"
              value={searchId}
              onChange={(e) => { setSearchId(e.target.value); setSearchName(''); }}
          ...
          <div className="flex-1">
            <label className="block text-sm font-medium text-foreground mb-1">Search Agent Name</label>
            <input
              type="text"
              value={searchName}
              onChange={(e) => { setSearchName(e.target.value); setSearchId(''); }}
      ...
      {/* Update Form */}
      {agentFound && (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 bg-card border border-border p-8 rounded-lg shadow-sm animate-in slide-in-from-bottom-4">
              ...
              {filterOpen && (
                <div className="absolute top-full right-0 mt-2 w-64 bg-white dark:bg-slate-800 border border-border rounded-md shadow-2xl z-50 p-4 max-h-96 overflow-y-auto">
                  <div className="flex justify-between items-center mb-4 border-b border-border pb-2">
                    <span className="text-sm font-semibold text-foreground">Visible Fields</span>
                    <button
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <button onClick={() => toggleGroupFields('core')} className="text-[10px] text-primary hover:underline">
                          {EDITABLE_FIELDS.core.every(f => visibleFields[f.id]) ? 'None' : 'All'}
                        </button>
                      </div>
                      {EDITABLE_FIELDS.core.map(field => (
                        <label key={field.id} className="flex items-center gap-2 mb-1 cursor-pointer">
                          <input
                            type="checkbox"
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <button onClick={() => toggleGroupFields('access')} className="text-[10px] text-primary hover:underline">
                          {EDITABLE_FIELDS.access.every(f => visibleFields[f.id]) ? 'None' : 'All'}
                        </button>
                      </div>
                      {EDITABLE_FIELDS.access.map(field => (
                        <label key={field.id} className="flex items-center gap-2 mb-1 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={!!visibleFields[field.id]}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-foreground/5 p-4 rounded-md border border-border/50">
              <div>
                <label className="block text-sm font-medium text-foreground/70 mb-1">Agent ID (Read Only)</label>
                <input
                  {...register('agent_id')}
                  readOnly
              ...
              <div>
                <label className="block text-sm font-medium text-foreground/70 mb-1">Agent Name (Read Only)</label>
                <input
                  {...register('agent_name')}
                  readOnly
            ...
            <div className="space-y-4 pt-2">
              {visibleFields.tools && (
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Access: Tools</label>
                  <Controller control={control} name="access_rights.tools" render={({ field }) => (
              ...
              {visibleFields.files && (
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Access: Files</label>
                  <Controller control={control} name="access_rights.files" render={({ field }) => (
                    <TagInput value={field.value} onChange={(newValue) => {
                      const processedTags = newValue
                        .flatMap(tag => tag.replace(/["']/g, "").split(/[,]+/)) // Split by space (\s) or comma (,)
                        .map(tag => tag.trim())              // Clean up whitespace
              ...
              {visibleFields.data_nodes && (
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Access: Data Nodes</label>
                  <Controller control={control} name="access_rights.data_nodes" render={({ field }) => (
                    <TagInput value={field.value} onChange={(newValue) => {
                      const processedTags = newValue
                        .flatMap(tag => tag.replace(/["']/g, "").split(/[,]+/)) // Split by space (\s) or comma (,)
                        .map(tag => tag.trim())              // Clean up whitespace
              ...
              {visibleFields.apis && (
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Access: APIs</label>
                  <Controller control={control} name="access_rights.apis" render={({ field }) => (
                    <TagInput value={field.value} onChange={(newValue) => {
                      const processedTags = newValue
                        .flatMap(tag => tag.replace(/["']/g, "").split(/[,]+/)) // Split by space (\s) or comma (,)
                        .map(tag => tag.trim())              // Clean up whitespace
              ...
              {visibleFields.servers && (
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Access: Servers</label>
                  <Controller control={control} name="access_rights.servers" render={({ field }) => (
                    <TagInput value={field.value} onChange={(newValue) => {
                      const processedTags = newValue
                        .flatMap(tag => tag.replace(/["']/g, "").split(/[,]+/)) // Split by space (\s) or comma (,)
                        .map(tag => tag.trim())              // Clean up whitespace
